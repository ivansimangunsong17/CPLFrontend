...
import { useJenisPenilaian } from "../../hooks/dosen/useJenisPenilaian";
import { useSubPenilaian } from "../../hooks/dosen/useSubPenilaian";
...
const DetailAturPenilaianDosen = () => {
    ...
    // PERBAIKAN: Hapus pemanggilan usePemetaanCPMK
    ...
    const isKelasLoading = kelasQuery.isLoading;

    const assessments = useMemo(() => jenisPenilaianQuery.data || [], [jenisPenilaianQuery.data]);

    // PERBAIKAN: Gunakan `subPenilaianQuery` yang benar
    const subAssessments = useMemo(() => subPenilaianQuery.data || [], [subPenilaianQuery.data]);

    // Status untuk Skeleton (hanya true saat data belum ada/cache)
    const isInitialDataLoading =
        jenisPenilaianQuery.isLoading ||
        subPenilaianQuery.isLoading;

    // Status untuk menonaktifkan tombol (true saat fetch awal ATAU refetch)
    const isDataRefetching =
        jenisPenilaianQuery.isFetching ||
        subPenilaianQuery.isFetching;
    // PERBAIKAN: Hapus pengecekan pemetaan
    // pemetaanCPMKQuery.isFetching;

    // Status untuk LoadingScreen
    const isMutating =
    ...
    // --- 3. State dan Handler ---
    // PERBAIKAN: Arahkan kembali ke rute Dosen
    const handleBack = () => navigate("/dashboard/dosen/atur_penilaian");
    ...
    const handleAddSubAssessment = () => {
        // PERBAIKAN: Hapus pengecekan 'isPemetaanLoading'
        if (selectedAssessmentId === null || !kelasId || isPenilaianLoading || isSubPenilaianLoading) {
            toast.warn("Data belum siap atau sedang memuat. Coba lagi sesaat.");
            return;
        }
        if (!newSubName) {
            toast.error("Terjadi kesalahan, nama baru tidak ditemukan.");
            return;
        }

        const parentAssessment = combinedAssessments.find(a => a.penilaian_id === selectedAssessmentId);
        if (!parentAssessment) {
            toast.error("Gagal menemukan jenis penilaian induk.");
            return;
        }
        const parentName = parentAssessment.nama_penilaian;
        const existingSubs = parentAssessment.subs;
        ...
        const newSubName = `${parentName} ${nextNumber}`;
    ...
    const removeAssessment = (assessment) => {
        setItemToDelete({
            isOpen: true,
            message: `Apakah Anda yakin ingin menghapus jenis penilaian "${assessment.nama_penilaian}"?`,
            onConfirm: () => {
                console.log("Menghapus penilaian (API Call needed), ID:", assessment.penilaian_id);
    ...
    const removeSub = (sub) => {
        setItemToDelete({
            isOpen: true,
            message: `Apakah Anda yakin ingin menghapus sub-penilaian "${sub.nama_sub_penilaian}"?`,
            onConfirm: () => {
                deleteSubPenilaianMutation.mutate({ sub_penilaian_id: sub.sub_penilaian_id }, {
    ...
    const closeConfirmModal = () => {
        setItemToDelete({ isOpen: false, message: "", onConfirm: () => { } });
    };
    ...
    return (
        <div className="p-6 bg-gray-50 min-h-screen">
                    ...
                    <tbody className="divide-y divide-gray-200">
                        {isInitialDataLoading ? (
                            ...
                            combinedAssessments.map((assessment) => {
                                ...
                                // PERBAIKAN: Hapus isPemetaanLoading
                                const isButtonDisabled = isDataRefetching || isMutating;
                                ...
                                return (
                                    <React.Fragment key={assessment.penilaian_id}>
                                        <tr className="hover:bg-gray-50">
                                            ...
                                            <td className={`px-6 py-4 ${subs.length === 0 ? 'italic text-gray-400' : ''}`}>
                                                {subs[0]?.nama_sub_penilaian || "-"}
                                            </td>
                                            <td className="px-6 py-4 text-center align-top">
                                                {subs[0] ? (
                                                    <button
                                                        onClick={() => removeSub(subs[0])}
                                                        disabled={isButtonDisabled}
                                        ...
                                        {subs.slice(1).map((sub) => (
                                            <tr key={sub.sub_penilaian_id} className="hover:bg-gray-50">
                                                <td className="px-6 py-4">{sub.nama_sub_penilaian}</td>
            ...
            <ConfModal
                ...
                description={itemToDelete.message}
                confirmText="Ya, Hapus"
                isLoading={deleteSubPenilaianMutation.isPending}
            />
        </div>
    );
};

export default DetailAturPenilaianDosen;