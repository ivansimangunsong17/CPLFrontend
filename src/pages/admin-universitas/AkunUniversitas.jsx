...
const AkunUniversitas = () => {
  ...
  const [data, setData] = useState(
    Array(10).fill(null).map((_, index) => ({
      id: index + 1,
      nama: `Contoh Pengguna${index + 1}`,
      email: `pengguna${index + 1}@example.com`,
      hakAkses: index % 2 === 0 ? "Admin Universitas" : "Admin Prodi",
  ...
  const formFields = [
    ...
    },
    {
      ...
      label: "Hak Akses",
      type: "select",
      options: ["Admin Universitas", "Admin Prodi"],
      required: true,
    },
    {
      name: "status",
            ...
            <tbody className="divide-y divide-gray-200">
              {data.map((item) => (
                <tr key={item.id} className="hover:bg-gray-50 transition">
                  ...
                  <td className="p-4">
                    <span className={`px-3 py-1 rounded-full text-xs font-medium ${item.hakAkses === "Admin Universitas"
